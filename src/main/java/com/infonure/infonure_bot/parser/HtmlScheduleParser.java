...
public class HtmlScheduleParser {
    private static final Logger log = LoggerFactory.getLogger(HtmlScheduleParser.class);
    ...
    public Schedule parse(InputStream htmlInputStream, String baseUri) throws IOException {
        ...
        if (mainTable == null) {
            log.error("Основну таблицю MainTT не знайдено.");
            return schedule;
        }
            ...
            if (groupNameCell != null && currentTimeSlot != null && !weekNumbersHeader.isEmpty()) {
                ...
                for (Element lessonCell : lessonCells) {
                    if (weekIndex >= weekNumbersHeader.size()) {
                        ...
                        log.warn("Виявлено більше комірок із заняттями, ніж тижнів у заголовку для групи {} в таймслоті {}", groupNameForRow, currentTimeSlot.getLessonNumber());
                        break;
                    }
                    ...
                    if (!colspanAttr.isEmpty()) {
                        try {
                            ...
                            log.error("Помилка парсингу colspan: {}", colspanAttr);
                    ...
                    for (int k = 0; k < colspan; k++) {
                        if (weekIndex < weekNumbersHeader.size()) {
                            ...
                            log.warn("Colspan виходить за межі кількості тижнів.");
                            break;
                        }
                    }
                }
